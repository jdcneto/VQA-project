...
def create_questions(shape, color, location, image_id):
  ...
  for l in Location:
    ...
    neg_answer = 'no' if l is location else 'yes'
    yes_no_questions.append((f'is there not a shape in the {cur_location_name}?', neg_answer))
    yes_no_questions.append((f'is there not a shape in the {cur_location_name} in the image?', neg_answer))
    yes_no_questions.append((f'does the image not contain a shape in the {cur_location_name}?', neg_answer))
    yes_no_questions.append((f'is a shape not present in the {cur_location_name}?', neg_answer))
